...
public class CarvedLargePumpkinSliceBlock extends LargeJackOLanternSliceBlock {
	...
	public ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
		if (stack.getItemHolder().getKey() != null) {
			...
			if (jackolantern != null && jackolantern.largeJackOLantern() != null) {
				if (jackolantern.largeJackOLantern().value() instanceof LargeJackOLanternSliceBlock slice) {
					...
					if (canCarve(hitface, facing) && (hitface.getAxis() == Axis.X && carvedside == CarvedSide.X || hitface.getAxis() == Axis.Z && carvedside == CarvedSide.Z)) {
						if (!level.isClientSide()) {
							BlockState blockstate = slice.withPropertiesOf(state);
							if (slice == AutumnityBlocks.LARGE_REDSTONE_JACK_O_LANTERN_SLICE.get()) {

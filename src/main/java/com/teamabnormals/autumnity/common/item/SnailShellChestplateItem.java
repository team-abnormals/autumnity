...
public class SnailShellChestplateItem extends ArmorItem {
	...
	public void appendHoverText(ItemStack stack, Item.TooltipContext context, List<Component> tooltip, TooltipFlag tooltipFlag) {
		...
		tooltip.add(Component.translatable(MobEffects.DAMAGE_RESISTANCE.value().getDescriptionId()).withStyle(ChatFormatting.BLUE).append(" ").append(Component.translatable("potion.potency.2").withStyle(ChatFormatting.BLUE)));
		tooltip.add((Component.translatable("attribute.modifier.plus." + Operation.ADD_VALUE.id(), ItemAttributeModifiers.ATTRIBUTE_MODIFIER_FORMAT.format(10), Component.translatable(Attributes.KNOCKBACK_RESISTANCE.value().getDescriptionId()))).withStyle(ChatFormatting.BLUE));
	}
}
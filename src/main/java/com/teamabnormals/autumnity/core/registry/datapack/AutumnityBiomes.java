...
public class AutumnityBiomes {
	...
	private static Biome mapleForest(HolderGetter<PlacedFeature> features, HolderGetter<ConfiguredWorldCarver<?>> carvers) {
		...
		generation.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, AutumnityPlacedFeatures.MAPLE_FOREST_FALLEN_LEAVES);
		generation.addFeature(GenerationStep.Decoration.VEGETAL_DECORATION, AutumnityPlacedFeatures.PATCH_FOUL_BERRY_BUSH);
		return (new Biome.BiomeBuilder()).hasPrecipitation(true).temperature(0.5F).downfall(0.8F).specialEffects((new BiomeSpecialEffects.Builder()).waterColor(6856341).waterFogColor(6987399).fogColor(12638463).skyColor(calculateSkyColor(0.7F)).foliageColorOverride(0xA1C440).grassColorOverride(0xAAB53B).ambientMoodSound(AmbientMoodSettings.LEGACY_CAVE_SETTINGS).backgroundMusic(OverworldBiomes.NORMAL_MUSIC).build()).mobSpawnSettings(spawns.build()).generationSettings(generation.build()).build();
	}
	...
	private static Biome pumpkinFields(HolderGetter<PlacedFeature> features, HolderGetter<ConfiguredWorldCarver<?>> carvers) {
		...
		return (new Biome.BiomeBuilder()).hasPrecipitation(true).temperature(0.6F).downfall(0.4F).specialEffects((new BiomeSpecialEffects.Builder()).waterColor(6136035).waterFogColor(4620239).fogColor(12638463).skyColor(calculateSkyColor(0.8F)).foliageColorOverride(0xA1C440).grassColorOverride(0xAAB53B).ambientMoodSound(AmbientMoodSettings.LEGACY_CAVE_SETTINGS).backgroundMusic(OverworldBiomes.NORMAL_MUSIC).build()).mobSpawnSettings(spawns.build()).generationSettings(generation.build()).build();
	}
